...
pytestmark = pytest.mark.skipif(
    not os.path.exists('/opt/ros/jade'),
    reason="requires ROS jade distro"
)
...
def test_rospy_import(setup):
    ...
    try:
        ...
        import pyros_setup.jade

        # Loading default configuration
        pyros_setup.jade.configure({'WORKSPACES:[]'})

        # Do the setup as expected by ROS
        pyros_setup.jade.activate()

        import rospy

    assert rospy is not None
    assert rospy.__file__ == '/opt/ros/jade/lib/python2.7/dist-packages/rospy/__init__.pyc'
