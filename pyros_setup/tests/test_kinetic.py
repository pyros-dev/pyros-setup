...
pytestmark = pytest.mark.skipif(
    not os.path.exists('/opt/ros/kinetic'),
    reason="requires ROS kinetic distro"
)
...
def test_rospy_import(setup):
    ...
    try:
        ...
        import pyros_setup.kinetic

        # Loading default configuration
        pyros_setup.kinetic.configure({'WORKSPACES:[]'})

        # Do the setup as expected by ROS
        pyros_setup.kinetic.activate()

        import rospy

    assert rospy is not None
    assert rospy.__file__ == '/opt/ros/kinetic/lib/python2.7/dist-packages/rospy/__init__.pyc'
